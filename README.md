...
`target=stash` 使用完全独立的基础模板、远程资源模型和节点生成器，不复用 Clash/Mihomo 的 `proxyToClash` 或 provider 扩展字段。远程订阅按 Stash [Proxy Providers](https://stash.wiki/en/proxy-protocols/proxy-providers) 规范输出 `url`、`path`、`interval` 和按需使用的 `headers`，节点筛选条件由策略组的 `filter` 表达，并由策略组通过 `use` 引用；上游响应必须是顶层包含 `proxies` 的 Stash YAML。服务商若仍返回普通 Base64 节点列表，需要自行根据 Stash 请求头提供兼容内容，本项目不会假定任意订阅都能被客户端直接读取。远程规则集使用独立的 Stash [Rule Set](https://stash.wiki/en/rules/rule-set) 投影：`clash-domain:` 与 `clash-ipcidr:` 可直接生成 YAML、text 或 MRS `rule-provider`，`clash-classic:` 仅接受 YAML 或 text。`.mrs`、`.yaml`、`.yml` 可由无歧义后缀确定格式；历史 `.txt`/`.list` 内容可能是 YAML，也可能是纯文本，只有显式追加 `|stash-format=text` 才会交给 Stash 原生读取，未声明或使用其他后缀时继续复用服务端旧抓取流程。已确认格式的 URL 由 Stash 更新，转换服务器不会预抓取。格式声明冲突、classical MRS、Stash 不支持的展开规则或悬空引用会失败关闭，不会静默删除。直接节点依据 Stash 当前[协议字段](https://stash.wiki/en/proxy-protocols/proxy-types)逐项投影；无法完整表示的字段或组合会明确跳过。旧 INI、YAML、TOML 配置不需要新增 `stash_rule_base`，缺省时自动使用 `base/stash.yaml`。
