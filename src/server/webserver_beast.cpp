...
void appendExposeHeader(http::response<http::string_body> &response,
...
void BeastSession::stopAccepting() {
...
void BeastSession::onRead(beast::error_code error, std::size_t) {
  ...
  if (error) {
    ...
    context_ = std::make_shared<RequestContext>(request_id_, started_at_);
